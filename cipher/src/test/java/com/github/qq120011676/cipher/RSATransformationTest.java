...
public class RSATransformationTest {
    static {
        Security.addProvider(new BouncyCastleProvider());
    }

    public static void main(String[] args) throws Exception {
        String phone = "15828395524";
        for (int i = 0; i < 10; i++) {
            phone += phone;
        }
        System.out.println(phone.length());
        //phone = "120011676@qq.com";
        ...
        rsa.init(Cipher.ENCRYPT_MODE, getPrivateKey(privateKey));
        rsa.doFinal(phone.getBytes());
        byte[] bs = rsa.doFinal(phone.getBytes());
